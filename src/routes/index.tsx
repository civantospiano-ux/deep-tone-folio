...
import { Play, Instagram, Mail, X, ArrowUpRight, Menu } from "lucide-react";
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-colors duration-500 ${
        scrolled || menuOpen ? "bg-background/85 backdrop-blur-md border-b border-border/40" : "bg-transparent"
      }`}
    >
      <nav className="mx-auto flex max-w-[1600px] items-center justify-between px-6 py-5 md:px-12 md:py-7">
        ...
        <a
          href="#work"
          className="hidden font-display text-[10px] tracking-brutal text-foreground transition-colors hover:text-accent md:block"
        >
          Listen
        </a>
        <button
      ...
      {menuOpen && (
        <div className="border-t border-border/30 bg-background px-6 pb-8 pt-4 md:hidden">
          <ul className="flex flex-col gap-6">
            {items.map(([label, href]) => (
              <li key={href}>
                <a
                  ...
                  className="font-display text-[13px] tracking-brutal text-muted-foreground transition-colors hover:text-accent"
                >
                  {label}
                </a>
              </li>
            ...
            <li>
              <a
                href="#work"
                onClick={() => setMenuOpen(false)}
                className="font-display text-[13px] tracking-brutal text-foreground transition-colors hover:text-accent"
              >
                Listen
              </a>
            </li>
